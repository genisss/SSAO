...
void GraphicsSystem::init(int window_width, int window_height, std::string assets_folder) {
	...
	ssao_shader_ = new Shader("data/shaders/deferred.vert", "data/shaders/ssao.frag");
	
	blur_shader_ = new Shader("data/shaders/deferred.vert", "data/shaders/blur.frag");

	gbuffer_.initGbuffer(window_width, window_height);

	ssaobuffer_.initSSAO(window_width, window_height);

	blurbuffer_.initBlur(window_width, window_height);
	...
	for (unsigned int i = 0; i < 64; ++i)
	{
		...
		sample *= (0.1f + scale*scale * (1.0f - 0.1f));
		ssaoKernel.push_back(sample);
	}

	std::vector<lm::vec3> ssaoNoise;
	for (unsigned int i = 0; i < 16; i++) {
		lm::vec3 noise(randomFloats(generator) * 2.0 - 1.0, randomFloats(generator) * 2.0 - 1.0, 0.0f);
		ssaoNoise.push_back(noise);
	}
	//generate new openGL texture and bind it (tell openGL we want to do stuff with it)
	glGenTextures(1, &noiseTexture);
	glBindTexture(GL_TEXTURE_2D, noiseTexture); //we are making a regular 2D texture
...
void GraphicsSystem::update(float dt) {
	...
	ssaobuffer_.bindAndClear();
	//bindAndClearScreen_();
	resetShaderAndMaterial_();
	renderSSAObuffer();

	blurbuffer_.bindAndClear();
	resetShaderAndMaterial_();
	renderBlurbuffer();

	/////////////////////////////////////////////////////////////////////////// DESCOMENTAR AIXOOOO DE SOTAAAAAAA
	/* SCREEN PASS */
	bindAndClearScreen_();
	resetShaderAndMaterial_();
    renderGbuffer();
	...
	glViewport(2 * GLsizei(viewport_width_ / 4), 0, GLsizei(viewport_width_ / 4), GLsizei(viewport_height_ / 4));
	screen_space_shader_->setTexture(U_SCREEN_TEXTURE, gbuffer_.color_textures[2], 0);
	geometries_[screen_space_geom_].render();

	glViewport(3 * GLsizei(viewport_width_ / 4), 0, GLsizei(viewport_width_ / 4), GLsizei(viewport_height_ / 4));
	screen_space_shader_->setTexture(U_SCREEN_TEXTURE, blurbuffer_.color_textures[0], 0);
	geometries_[screen_space_geom_].render();
...
void GraphicsSystem::renderGbuffer() {
	...
	deferred_shader_->setUniform(U_VIEW_MATRIX, cam.view_matrix);
	...
	deferred_shader_->setTexture(U_TEX_SSAO, blurbuffer_.color_textures[0], 12);
...
void GraphicsSystem::renderSSAObuffer() {
	useShader(ssao_shader_);

	//upload the uniforms
	ssao_shader_->setTexture(U_TEX_POSITION, gbuffer_.color_textures[0], 8);
	ssao_shader_->setTexture(U_TEX_NORMAL, gbuffer_.color_textures[1], 9);
	ssao_shader_->setTexture(U_TEX_NOISE, noiseTexture, 11);
	ssao_shader_->setUniform(U_PROJECTION_MATRIX, ECS.getComponentInArray<Camera>(ECS.main_camera).projection_matrix);
	ssao_shader_->setUniform(U_RANDOM_SAMPLES, ssaoKernel[0], ssaoKernel.size());

	geometries_[screen_space_geom_].render();

	glBindFramebuffer(GL_READ_FRAMEBUFFER, ssaobuffer_.framebuffer);
	glBindFramebuffer(GL_DRAW_FRAMEBUFFER, 0);
	glBlitFramebuffer(0, 0,
		viewport_width_, viewport_height_,
		0, 0,
...
void GraphicsSystem::renderBlurbuffer() {
	useShader(blur_shader_);


	//upload the uniforms
	blur_shader_->setTexture(U_TEX_SSAO, ssaobuffer_.color_textures[0], 12);
	

	geometries_[screen_space_geom_].render();

	glBindFramebuffer(GL_READ_FRAMEBUFFER, blurbuffer_.framebuffer);
	glBindFramebuffer(GL_DRAW_FRAMEBUFFER, 0);
	glBlitFramebuffer(0, 0,
		viewport_width_, viewport_height_,
		0, 0,
...
void GraphicsSystem::renderMeshComponent_(Mesh& comp) {
	...
	shader_->setUniform(U_VIEW_MATRIX, cam.view_matrix);
