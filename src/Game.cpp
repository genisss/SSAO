...
void Game::init(int w, int h) {
	...
	int sphere_entity2 = ECS.createEntity("phong_sphere");
	Transform& st2 = ECS.getComponentFromEntity<Transform>(sphere_entity2);
	//st.translate(1.0f, 6.0f, 5.0f);
	//for suit
	st2.scale(0.5, 0.5, 0.5);
	st2.rotate(90*DEG2RAD, lm::vec3(1,0,0));
	st2.translate(3, 1, 0.0);
	Mesh& sphere_mesh2 = ECS.createComponentForEntity<Mesh>(sphere_entity2);
	sphere_mesh2.geometry = suit_geom;
	sphere_mesh2.material = mat_blue_check_index;
